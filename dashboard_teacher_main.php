...
// --- Data for Correction Module (from your previous file) ---
$correction_classes = [];
$correction_subjects = [];
...
try {
    ...
    // --- Data for Correction Upload Form (from your previous file) ---
    $correction_classes = $filter_classes;
    $correction_subjects = $filter_subjects;
    
    // --- Fetch Correction Paper Statuses (from your previous file) ---
    $stmt_cp_list = $pdo->query("
        SELECT 
            ...
            cp.status as correction_status -- Added alias to avoid conflict
    ...
    // (Charts 1, 2, 3 logic remains the same as your file)
    // ... (Chart logic omitted for brevity, it's unchanged) ...
    $sql_total_assigned = "
    ...
    foreach ($all_subjects_raw as $row) {
        ...
        if (!isset($modal_details[$className])) {
            $modal_details[$className] = ['uploaded' => [], 'not_uploaded' => []];
        }
    ...
    $sql_class_chart = "
        SELECT c.name as class_name, p.status, COUNT(p.paper_id) as count
        FROM papers p
        JOIN assignments a ON p.assignment_id = a.assignment_id
        JOIN classes c ON a.class_id = c.class_id
        $sql_where_clause
        GROUP BY c.name, p.status ORDER BY c.name
    ";
    ...
    $sql_detailed_chart = "
        SELECT c.name as class_name, s.name as subject_name, p.status, COUNT(p.paper_id) as count
        FROM papers p
        JOIN assignments a ON p.assignment_id = a.assignment_id
        JOIN classes c ON a.class_id = c.class_id
        JOIN subjects s ON a.subject_id = s.subject_id
        $sql_where_clause
        GROUP BY c.name, s.name, p.status ORDER BY c.name, s.name
    ";
    ...
    $sql_base = "
        SELECT 
            ...
            p.file_path,
            p.status,
            p.submission_type, -- Get submission type
...
<main class="flex-1 p-6 sm:p-10 relative">
    ...
    <!-- Correction Module (from your previous file) -->
                                ...
                                <tr>
                                    ...
                                    <td class="px-4 py-4 whitespace-nowrap">
                                        <?php
                                            ...
                                            if ($cp['correction_status'] === 'no_correction') {
                                                $cp_status_color = 'bg-green-100 text-green-800';
                                                $cp_status_text = 'No Correction Required';
                                            } elseif ($cp['correction_status'] === 'corrected') {
                                                $cp_status_color = 'bg-blue-100 text-blue-800';
                                                $cp_status_text = 'Corrected & Reviewed';
                                            } elseif ($cp['correction_status'] === 'completed') {
                                                $cp_status_color = 'bg-gray-500 text-white';
                                                $cp_status_text = 'Completed';
                                            } else { // pending_review
                                    ...
                                    <td class="px-4 py-4 text-sm text-gray-500">
                                        <?php if ($cp['correction_status'] === 'corrected'): ?>
                                            <div class="flex items-center space-x-2">
                                                ...
                                                <form action="admin_correction_action.php" method="POST" class="inline">
                                                    <input type="hidden" name="action" value="mark_completed">
                                                    <input type="hidden" name="correction_paper_id" value="<?php echo $cp['correction_paper_id']; ?>">
                                                    <button type="submit" class="text-green-600 hover:text-green-900 text-sm font-medium">
                                                        Mark Completed
                                                    </button>
                                                </form>
                                            </div>
                                        <?php elseif ($cp['correction_status'] === 'no_correction'): ?>
                                            Marked as correct.
                                        <?php elseif ($cp['correction_status'] === 'completed'): ?>
                                            <span class="text-green-700 font-medium">Completed</span>
                                        <?php else: // pending_review ?>
                                            Pending review...
                                        <?php endif; ?>
                                    </td>
                                </tr>
    ...
    <!-- Paper Submissions Table (MODIFIED) -->
            ...
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th scope="col" class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">File / View</th>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    <?php if (empty($papers)): ?>
                        <tr>
                            <td colspan="6" class="px-4 py-4 text-sm text-gray-500 text-center">No papers match your filter criteria.</td>
                        </tr>
                    <?php else: ?>
                        <?php foreach ($papers as $paper): ?>
                            <tr>
                                ...
                                <td class="px-4 py-4 whitespace-nowrap">
                                    <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full <?php echo $paper['submission_type'] === 'text' ? 'bg-purple-100 text-purple-800' : 'bg-blue-100 text-blue-800'; ?>">
                                        <?php echo $paper['submission_type'] === 'text' ? 'Typed' : 'File'; ?>
                                    </span>
                                </td>
    ...
    <!-- (All Modals and Chart JS script from your file remain unchanged) -->
    <!-- Detail Modal -->
            ...
            <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                <div class="sm:flex sm:items-start">
                    <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
                        ...
                        <div class="mt-4 max-h-64 overflow-y-auto" id="modal-content"></div>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                <div class="sm:flex sm:items-start">
                    <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
                        ...
                        <div class="mt-4 max-h-64 overflow-y-auto" id="feedback-modal-content"></div>
                    </div>
                </div>
            </div>
            <div class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        // (All modal and chart JS from your file remains unchanged)
        // --- Modal Handling ---
        ...
        function showFeedbackModal(teacher, notes, imagePath) {
            ...
            html += `<p class="text-sm text-gray-600 mt-1 whitespace-pre-wrap">${notes.replace(/<br\s*\/?>/mg,"")}</p>`;
            html += '</div>';
            if (imagePath) {
                html += '<div>';
