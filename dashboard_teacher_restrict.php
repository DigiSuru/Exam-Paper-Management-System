...
$isPastDeadline = $currentTime > $cutoffTime;

// --- EXEMPTION LOGIC ---
// Normalize the name to lowercase and trim whitespace for comparison
$loggedInName = strtolower(trim($_SESSION['name'] ?? ''));
$isExempt = ($loggedInName === 'nidhi pareek');

$blocked_message = "";

// Only show blocked message if deadline passed AND user is NOT exempt
if ($isPastDeadline && !$isExempt) {
    $blocked_message = "Upload disabled for classes above 5th after Nov 26, 9:00 PM.";
}
// ------------------------------
...
function getGradeFromClassName($className) {
    // Look for a number in the class name (e.g., "Class 5", "10th", "Grade 1")
    if (preg_match('/(\d+)/', $className, $matches)) {
        return intval($matches[1]);
    }
    // If no number (e.g., "Nursery", "LKG", "Prep"), treat as grade 0 (allowed)
    return 0;
}
...
<main class="flex-1 p-6 sm:p-10">
    ...
    <!-- Only show notice if user is NOT exempt -->
    <?php if ($isPastDeadline && !$isExempt): ?>
        <div class="mb-6 rounded-md bg-yellow-50 border-yellow-400 text-yellow-800 border p-4 flex items-center">
            <svg class="h-5 w-5 text-yellow-400 mr-2" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z" clip-rule="evenodd"/></svg>
            <p><strong>Notice:</strong> <?php echo $blocked_message; ?></p>
        </div>
    <?php endif; ?>
                        ...
                        <?php else: ?>
                            <?php foreach ($teacher_assignments as $assignment): ?>
                                <?php
                                    ...
                                    $grade = getGradeFromClassName($assignment['class_name']);
                                    $isDisabled = false;
                                    $optionText = htmlspecialchars($assignment['class_name'] . ' - ' . $assignment['subject_name']);
                                    
                                    // Apply restriction only if user is NOT exempt
                                    if ($isPastDeadline && $grade > 5 && !$isExempt) {
                                        $isDisabled = true;
                                        $optionText .= " (Closed: > 5th Class)";
                                    }
                                ?>
                                <option value="<?php echo $assignment['assignment_id']; ?>" <?php echo $isDisabled ? 'disabled' : ''; ?>>
                                    <?php echo $optionText; ?>
                                </option>
                            <?php endforeach; ?>
                        <?php endif; ?>
